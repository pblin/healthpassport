...
if config_needed: 
    ...
    if(config.has_option('BOT', 'maxloansize')):
        maxLoanSize = Decimal(config.get("BOT",'maxloansize'))
...
def returnTradeHistory(currencyPair = "all"):
    msg = bot.returnTradeHistory(currencyPair)
    log.log("START: Trade History")
    log.log( str(msg) )
    for trade in msg:
        log.log(" Trade " + str(trade))
    log.log("END: Trade History")
    
    
def createLoanOffer(cur,amt,rate):
    days = '2'
    #if (minDailyRate - 0.000001) < rate and Decimal(amt) > minLoanSize:
    #if float(amt) > minLoanSize and float(amt) <= maxLoanSize:
    if float(amt) > minLoanSize:
        ...
        # Added better logging for dryrun
        else:
            msg = {}
            msg["message"]="Dryrun only"
            log.offer(amt, cur, rate, days, msg)
    ...
    while activeCurIndex < len(lendingBalances):
        ...
        for offer in loans['offers']:
            ...
            while True:
                ...
                # Special logic to always lend more aggressively over sixty day threshold
                gapFactor = (gapBottom/100+(step/100*j))
                if Decimal(offer['rate']) >= sixtyDayThreshold:
                    gapFactor = (step/100*j)/2
                # end special logic 

                if s2 > activePlusLended*gapFactor and Decimal(offer['rate']) > curMinDailyRate:
                    j += 1
...
def sleepDelay(seconds):
    sys.stdout.flush()
    print("Sleeping for " + str(float(seconds/3600)) + " hours... ")
    time.sleep( seconds )
...
try:
    # Optional sleep for a defined time before starting up again (in case last withdrawal time is less than 24 hours ago)
    #sleepDelay(24 * 3600)
    while True:
        try:
            ...
            # Withdrawal limit in $ ($2k a day, but can reduce to a nominal amount e.g. 0.05 for testing purposes)
            poloniexwithdrawallimit = 2000
            ...
            for trade in trades:
                ...
                print("Latest XMR price " + str(xmrprice) + " xmr (as of " + str(xmrpricedate) + ")")
                break
            ...
            # Calculate amount to withdraw (including commission)
            xmramount = (poloniexwithdrawallimit / xmrprice)
            if (xmramount > 1):
                xmramount = int(xmramount)
            xmrestimatedwithdrawaltotal = xmramount * xmrprice
            xmramount = xmramount + 0.05
            
            
            # Withdraw
            print("Withdrawing estimated $" + str(xmrestimatedwithdrawaltotal))
            
            # !!!!!!! IMPORTANT : Uncomment this line to enable the bot   !!!!!!!!!!!!!!!!!
            retrycount = 30
            while(retrycount > 0):
                try:
                    bot.withdraw("XMR", xmramount, xmraddress)
                    print("Sent withdraw " + str(xmramount) + " XMR (including commission fee)) to ")
                    print("Withdraw address: " + str(xmraddress))
                    retrycount = 0
                except Exception as e:
                    retrycount = retrycount - 1
                    if('Withdrawal would exceed your daily withdrawal limit' in str(e)):
                        xmramount = xmramount - 0.5
                        retrydelay=2
                        print("Higher than daily withdrawal limit : Waiting " + str(retrydelay) + " seconds to retry with lower amount " + str(xmramount) + " XMR ...")
                        sleepDelay(retrydelay)
            ...
            if('Permission denied' in str(e)):
                ...
                # Deliberately do not exit on this error, because it means VPN has probably dropped, so if it reconnects then it will recover...
            if('Permission to withdraw denied' in str(e)):
                print "!!! Troubleshooting !!!"
                print "Have you enabled API to allow withdrawals ? Are you over the daily limit ?"
                # Deliberately don't exit on this error, because it might just be inside the daily limit, so waiting with recover
            sleepDelay(sleepdelay)            
            pass
except KeyboardInterrupt:
    if autoRenew == 1:
